...
const user = {
  ...
  addNew: async function (
    ...
    const vars =
      "(login_id, first_name, last_name, display_name, avatar_dirct)";
  ...
  getUserBysocketID: async function (socketID) {
    const result = await orm.findOne(
      "users LEFT JOIN login_info ON login_info.id = login_id",
      "users.id, users.display_name, users.avatar_dirct",
  ...
  updateSocketID: async function (loginID, socketID) {
    const change = `socketID = '${socketID}'`;
    const index = `login_id = ${loginID}`;
    await orm.updateOne(this.name, change, index);
  },
};
